-- Preserve confirmed-member access links (Girls T06 and Boys equivalent)
-- without weakening the post-auth confirmation rule for pending invitations.

alter table private.invitation_auth_states
...
begin
  if not exists (
    ...
    where conrelid='private.invitation_auth_states'::regclass
      and conname='invitation_auth_states_purpose_check'
  ) then
    alter table private.invitation_auth_states
      add constraint invitation_auth_states_purpose_check
      check (purpose in ('invite','access'));
  end if;
end $$;
...
declare
  ...
  v_purpose text;
begin
  if p_state_hash is null or p_state_hash !~ '^[0-9a-f]{64}$' then raise exception 'invalid invitation state'; end if;
  if p_product_key not in ('boys','girls') then raise exception 'invalid invitation product'; end if;
  v_expected_callback := case p_product_key when 'girls' then 'https://thegirlstripguide.com/invite-return.html' when 'boys' then 'https://theboystripguide.com/invite-return.html' end;
  if p_callback_url is distinct from v_expected_callback then raise exception 'invalid invitation callback'; end if;
  if p_expires_at <= now() or p_expires_at > now() + interval '15 minutes' then raise exception 'invalid invitation state expiry'; end if;
  ...
  if not found or v_trip.product_key is distinct from p_product_key then raise exception 'invitation product mismatch'; end if;

  if v_member.status in ('invited','opened') then
    v_purpose := 'invite';
  elsif v_member.status='confirmed' and v_member.user_id=p_intended_user_id then
    v_purpose := 'access';
  else
    raise exception 'invitation is not eligible';
  end if;

  if v_member.invite_token_hash is null then raise exception 'invitation token missing'; end if;
  if v_member.invite_token_expires_at is null or v_member.invite_token_expires_at <= now() then raise exception 'invitation token expired'; end if;
  if v_member.email is null then raise exception 'invitation email missing'; end if;
  select lower(email) into v_user_email from auth.users where id=p_intended_user_id;
  if v_user_email is null or v_user_email <> lower(v_member.email) then raise exception 'invitation user mismatch'; end if;
  ...
  insert into private.invitation_auth_states(
    state_hash,member_id,trip_id,product_key,intended_user_id,callback_url,
    invite_token_hash,expires_at,purpose
  ) values (
    p_state_hash,p_member_id,p_trip_id,p_product_key,p_intended_user_id,p_callback_url,
    v_member.invite_token_hash,p_expires_at,v_purpose
  );
end;
$$;
...
begin
  ...
  select * into v_state from private.invitation_auth_states where state_hash=p_state_hash for update;
  ...
  if v_state.product_key is distinct from p_expected_product or v_state.callback_url is distinct from p_callback_url or v_state.intended_user_id is distinct from p_user_id then raise exception 'invitation state mismatch'; end if;
  ...
  return jsonb_build_object('trip_id',v_state.trip_id,'product_key',v_state.product_key,'member_id',v_state.member_id,'purpose',v_state.purpose);
end;
$$;
