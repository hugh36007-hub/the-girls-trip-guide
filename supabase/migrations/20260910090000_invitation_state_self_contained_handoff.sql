-- The invitation handoff now carries the opaque server state together with the
-- one-time Supabase token hash in the URL fragment on the product origin.
-- Browser storage and server-side state guessing are no longer part of the flow.
...
begin
  ...
  if v_member.invite_token_expires_at is null
     or v_member.invite_token_expires_at <= v_now then
  ...
  if v_state.purpose='invite' then
    ...
    if v_member.status<>'confirmed' or v_member.user_id is distinct from p_user_id then
      raise exception 'confirmed access mismatch';
    end if;
    update public.trip_members
  ...
  set consumed_at=v_now,consumed_by=p_user_id
  where id=v_state.id;
