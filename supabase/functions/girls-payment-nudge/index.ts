...
Deno.serve(async req=>{
  ...
  try{
    ...
    const sender=character==='system'?'The Girls Trip Guide':character.charAt(0).toUpperCase()+character.slice(1)
    const delivery=await deliver({to:position.member.email,character,title:emailSubject,message,tripName:trip.name,cta:'View payment',url:target.toString(),subject:`${emailSubject} · ${trip.name}`,preheader:`${sender} · ${emailSubject}`,idempotencyKey:`gtg-${commId}-${recipientMemberId}`})
