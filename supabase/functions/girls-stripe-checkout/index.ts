...
async function stripeRequest(path:string,params?:URLSearchParams,idempotencyKey=''){
  for(let attempt=0;attempt<5;attempt++){
    const res=await fetch(`https://api.stripe.com/v1/${path}`,{method:params?'POST':'GET',headers:{Authorization:`Bearer ${env('STRIPE_SECRET_KEY')}`,...(params?{'Content-Type':'application/x-www-form-urlencoded'}:{}),...(idempotencyKey?{'Idempotency-Key':idempotencyKey}:{})},body:params?.toString()})
    const out=await res.json().catch(()=>({}));if(res.ok)return out
    if(params&&idempotencyKey&&res.status===409&&out?.error?.code==='idempotency_key_in_use'&&attempt<4){await new Promise(resolve=>setTimeout(resolve,150*(attempt+1)));continue}
    throw new Error(out?.error?.message||`Stripe error ${res.status}`)
  }
  throw new Error('Stripe checkout could not be created safely.')
}
...
async function reusablePending(tripId:string,userId:string){
  const rows=await service(`purchases?trip_id=eq.${encodeURIComponent(tripId)}&provider=eq.stripe&status=eq.pending&select=id,provider_checkout_id,created_at,purchased_by&order=created_at.desc&limit=5`)
  for(const row of Array.isArray(rows)?rows:[]){
    if(row?.purchased_by&&row.purchased_by!==userId){await service(`purchases?id=eq.${encodeURIComponent(row.id)}`,{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({status:'cancelled'})});continue}
    if(!row?.provider_checkout_id){const age=Date.now()-Date.parse(String(row?.created_at||''));if(Number.isFinite(age)&&age>15*60*1000){await service(`purchases?id=eq.${encodeURIComponent(row.id)}`,{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({status:'cancelled'})});continue}return{reservation:row}}
    try{const s=await stripeRequest(`checkout/sessions/${encodeURIComponent(row.provider_checkout_id)}`);if(s?.payment_status==='paid'){const result=await verifyAndActivate(s,userId);if(result.paid)return{alreadyActive:true,reference:s.id};if(result.refunded){await service(`purchases?id=eq.${encodeURIComponent(row.id)}`,{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({status:'refunded'})});continue}}
...
async function reservePending(tripId:string,userId:string){const body={trip_id:tripId,provider:'stripe',status:'pending',amount_pence:2499,currency:'GBP',purchased_by:userId};try{const rows=await service('purchases?select=id,provider_checkout_id,created_at,purchased_by',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify(body)});if(Array.isArray(rows)&&rows[0])return rows[0]}catch(error){const rows=await service(`purchases?trip_id=eq.${encodeURIComponent(tripId)}&provider=eq.stripe&status=eq.pending&select=id,provider_checkout_id,created_at,purchased_by&order=created_at.desc&limit=1`);if(Array.isArray(rows)&&rows[0])return rows[0];throw error}throw new Error('Secure checkout reservation could not be created.')}
async function attachPendingSession(reservation:any,session:any,userId:string){const rows=await service(`purchases?id=eq.${encodeURIComponent(reservation.id)}&status=eq.pending&select=id`,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify({provider_checkout_id:session.id,provider_payment_id:typeof session.payment_intent==='string'?session.payment_intent:null,amount_pence:Number(session.amount_total||2499),currency:String(session.currency||'gbp').toUpperCase(),purchased_by:userId})});if(!Array.isArray(rows)||!rows.length)throw new Error('Secure checkout reservation could not be attached to Stripe.')}
Deno.serve(async(req)=>{
  ...
  try{
    ...
    if(action==='create'){
      ...
      const email=clean(user?.email||body?.email,320);if(email&&/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))p.set('customer_email',email)
      const session=await stripeRequest('checkout/sessions',p,`full-trip-${reservation.id}`);await attachPendingSession(reservation,session,user.id);return json({ok:true,url:session.url,id:session.id})
    }
    if(action==='verify'){
      const sid=clean(body?.sessionId,200);if(!/^cs_/.test(sid))return json({error:'Invalid checkout session.'},400);const session=await stripeRequest(`checkout/sessions/${encodeURIComponent(sid)}`),tripId=clean(session?.metadata?.trip_id,80);if(!UUID.test(tripId))return json({error:'Checkout is not attached to a secure trip.'},400);await assertOwner(tripId,user.id);const result=await verifyAndActivate(session,user.id);if(result.refunded)return json({ok:false,paid:false,refunded:true,error:'This checkout was refunded and cannot be restored.'},409);if(!result.paid)return json({ok:false,paid:false},402);return json({ok:true,paid:true,persisted:result.persisted,tripId:result.tripId,reference:session.id,amount:Number(session.amount_total||2499)/100,currency:String(session.currency||'gbp').toUpperCase()})
    }
    if(action==='restore'){
