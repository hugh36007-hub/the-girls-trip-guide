...
async function stripeRequest(path:string, params?:URLSearchParams){
  ...
  if(!res.ok)throw new Error(out?.error?.message||`Stripe error ${res.status}`)
  return out
}
...
async function service(path:string,init:RequestInit={}){
  ...
  const text=await res.text();let out:any=null;try{out=text?JSON.parse(text):null}catch{out=text}
  if(!res.ok)throw new Error(typeof out==='object'&&out?.message?out.message:`Supabase error ${res.status}`)
  return out
}
...
async function upsertPurchase(input:{tripId:string,session:any,userId?:string|null,status?:string}){
  const body={trip_id:input.tripId,provider:'stripe',provider_checkout_id:input.session.id,
    provider_payment_id:typeof input.session.payment_intent==='string'?input.session.payment_intent:null,
    status:input.status||'paid',amount_pence:Number(input.session.amount_total||2499),
    currency:String(input.session.currency||'gbp').toUpperCase(),purchased_by:input.userId||null,
    completed_at:(input.status||'paid')==='paid'?new Date().toISOString():null}
  const rows=await service('purchases?on_conflict=provider_checkout_id&select=id,status',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=representation'},body:JSON.stringify(body)})
  return Array.isArray(rows)?rows[0]:rows
}
async function activateTrip(tripId:string,purchaseId:string){
  const now=new Date().toISOString()
  for(const entitlement of ['full_trip','evidence','vault','full_comms']){
    await service('trip_entitlements?on_conflict=trip_id,entitlement',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=minimal'},body:JSON.stringify({trip_id:tripId,entitlement,source_purchase_id:purchaseId,active:true,granted_at:now,revoked_at:null})})
  }
...
async function verifyAndActivate(session:any,expectedOwnerId?:string){
  const valid=session?.payment_status==='paid'&&session?.metadata?.product==='the-full-trip'&&session?.metadata?.product_key==='girls'
  if(!valid)return {paid:false,persisted:false}
  const tripId=clean(session?.metadata?.trip_id,80),userId=clean(session?.metadata?.purchaser_user_id,80)||null
  if(!UUID.test(tripId))return {paid:true,persisted:false,tripId:null}
  if(expectedOwnerId){
    await assertOwner(tripId,expectedOwnerId)
    if(userId&&userId!==expectedOwnerId)throw new Error('Checkout ownership does not match this account.')
  }
  const purchase=await upsertPurchase({tripId,session,userId:userId||expectedOwnerId||null,status:'paid'})
  if(purchase?.id){await activateTrip(tripId,purchase.id);return {paid:true,persisted:true,tripId}}
  return {paid:true,persisted:false,tripId}
}
async function ensureWebhook(){
  try{
    const list=await stripeRequest('webhook_endpoints?limit=100')
    const existing=(list?.data||[]).find((x:any)=>x?.url===WEBHOOK_URL)
    const required=['checkout.session.completed','checkout.session.async_payment_succeeded','charge.refunded']
    if(existing?.id){
      const enabled=new Set(Array.isArray(existing.enabled_events)?existing.enabled_events:[])
      if(!required.every(event=>enabled.has(event))){
        const update=new URLSearchParams()
        for(const event of required)update.append('enabled_events[]',event)
        await stripeRequest(`webhook_endpoints/${encodeURIComponent(existing.id)}`,update)
      }
      return existing.id
    }
    const p=new URLSearchParams();p.set('url',WEBHOOK_URL);for(const event of required)p.append('enabled_events[]',event);p.set('description','The Girls Trip Guide checkout activation and refund handling')
    const created=await stripeRequest('webhook_endpoints',p)
    return created?.id||null
  }catch(error){console.warn('Webhook registration check failed',error);return null}
}
async function currentEntitlement(tripId:string){
  const rows=await service(`trip_entitlements?trip_id=eq.${encodeURIComponent(tripId)}&entitlement=eq.full_trip&active=eq.true&select=trip_id`)
...
async function reusablePending(tripId:string,userId:string){
  ...
  for(const row of Array.isArray(rows)?rows:[]){
    if(!row?.provider_checkout_id)continue
    try{
      const s=await stripeRequest(`checkout/sessions/${encodeURIComponent(row.provider_checkout_id)}`)
...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    if(!user?.id)return json({error:'Please sign in again before using secure checkout.'},401)
    ...
    if(action==='create'){
      ...
      if(pending?.url)return json({ok:true,url:pending.url,id:pending.id,reused:true})
      await ensureWebhook()
      const params=new URLSearchParams()
      params.set('mode','payment');params.set('line_items[0][price]',PRICE_ID);params.set('line_items[0][quantity]','1')
    ...
    if(action==='verify'){
      ...
      if(!/^cs_/.test(sessionId))return json({error:'Invalid checkout session.'},400)
      const session=await stripeRequest(`checkout/sessions/${encodeURIComponent(sessionId)}`)
      const tripId=clean(session?.metadata?.trip_id,80)
      if(!UUID.test(tripId))return json({error:'Checkout is not attached to a secure trip.'},400)
      await assertOwner(tripId,user.id)
      const result=await verifyAndActivate(session,user.id)
      if(!result.paid)return json({ok:false,paid:false},402)
      return json({ok:true,paid:true,persisted:result.persisted,tripId:result.tripId,reference:session.id,amount:Number(session.amount_total||2499)/100,currency:String(session.currency||'gbp').toUpperCase()})
    }

    if(action==='restore'){
      const tripId=clean(body?.tripId,80)
      if(!UUID.test(tripId))return json({ok:true,paid:false})
      await assertOwner(tripId,user.id)
      if(await currentEntitlement(tripId))return json({ok:true,paid:true,restored:true})
      const pending=await reusablePending(tripId,user.id)
      if(pending?.alreadyActive)return json({ok:true,paid:true,restored:true,reference:pending.reference})
