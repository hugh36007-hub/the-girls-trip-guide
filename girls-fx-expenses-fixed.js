/* Girls local-currency expenses: corrected runtime. */
(()=>{
'use strict';
if(window.__GTG_FX_EXPENSES_FIXED__)return;
window.__GTG_FX_EXPENSES_FIXED__=true;
...
const FX=`${SUPABASE_URL}/functions/v1/girls-fx-rate`;
...
function tripId(){return new globalThis.URL(location.href).searchParams.get('trip_id')||''}
...
async function waitForSession(c,attempts=20){for(let i=0;i<attempts;i+=1){const {data,error}=await c.auth.getSession();if(error)throw error;if(data?.session?.access_token)return data.session;if(i<attempts-1)await sleep(100)}return null}
function recordId(form){return form?.querySelector?.('[name="__gtg_record_id"],[name="id"]')?.value||''}
function dedupePeople(form){const select=form?.querySelector?.('select[name="people"][multiple]');if(!select)return;const seen=new Set();[...select.options].forEach(o=>{const key=String(o.value||'').trim();if(!key||seen.has(key))o.remove();else seen.add(key)})}
...
function inferCurrency(destination){
 ...
 if(has('north cyprus','northern cyprus','kyrenia','girne','famagusta','gazimağusa','gazimagusa','lefkoşa'))return'TRY';
...
async function fxRate(currency,amount){
 ...
 const cached=rateCache.get(code);if(cached)return{...cached,gbp_amount:Number(amount||0)*cached.rate};
 ...
 const out=await res.json().catch(()=>({}));if(!res.ok||!Number(out.rate))throw Error(out.error||'Exchange rate unavailable.');const value={rate:Number(out.rate),rate_date:String(out.rate_date||new Date().toISOString().slice(0,10)),source:String(out.source||'FX rate')};rateCache.set(code,value);return{...value,gbp_amount:Number(amount||0)*value.rate};
}
...
if(window.__GTG_FX_TEST__)window.__GTG_FX_TEST_HOOKS__={tripId,inferCurrency,effectiveCurrency};
})();
