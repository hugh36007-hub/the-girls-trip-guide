...
function tripRole(){
  ...
  const ownerControl=document.querySelector('[data-panel="money"] [data-a="addExpense"]');
  if(ownerControl)rememberedRole='owner';
  else if(!rememberedRole)rememberedRole='member';
  return rememberedRole==='owner'?'owner':'member';
}
function ownerExpenseButton(){
  const b=document.createElement('button');b.type='button';b.dataset.roleExpense='1';b.setAttribute('aria-label','Add expense');return b;
}
...
function decorate(button,kind,label,aria=label){
  ...
  if(button.dataset.option7Icon===kind&&button.querySelector('.dock-icon')&&labelNode){if(labelNode.textContent!==label)labelNode.textContent=label;return;}
  const icon=document.createElement('span');icon.className='dock-icon';icon.setAttribute('aria-hidden','true');icon.innerHTML=ICONS[kind];
  const small=document.createElement('small');small.className='dock-label';small.textContent=label;
  button.replaceChildren(icon,small);button.dataset.option7Icon=kind;
}
function normaliseDock(){
  ...
  const active={overview,plan,group,evidence}[action]||overview;if(active)active.classList.add('active');
...
const style=document.createElement('style');style.id='gtg-option7-dock-style';style.textContent=`
...
.dock.gtg-option7 .dock-icon{display:block;width:23px;height:23px;font-size:0;line-height:0}.dock.gtg-option7 .dock-icon svg{display:block;width:100%;height:100%}
...
document.addEventListener('click',event=>{
  ...
  if(expense){event.preventDefault();event.stopImmediatePropagation();const existing=document.querySelector('[data-panel="money"] [data-a="addExpense"]');if(existing){existing.click();return;}const money=document.querySelector('nav.dock button[data-tab="money"]');money?.click();setTimeout(()=>document.querySelector('[data-panel="money"] [data-a="addExpense"]')?.click(),0);return;}
