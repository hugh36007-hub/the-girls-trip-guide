...
assert(home.includes('photoHold.ready=true')&&home.includes('if(shouldOpen)setTimeout(openHidden,0)'),'four-second hold must arm first and open after pointer release');
...
assert(html.includes('/girls-app-v2.js?v=10')&&html.includes('/girls-hero-vault-ux.js?v=4')&&html.includes('/girls-performance-loader.js?v=18'),'Updated scripts must be cache-busted');
assert(sw.includes("gtg-pwa-v37-hidden-gallery-touch")&&sw.includes('/girls-performance-loader.js?v=18'),'PWA cache generation must include the updated loader');
