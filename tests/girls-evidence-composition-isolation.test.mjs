...
try{
 ...
 assert.equal(full.composition,'full');for(const src of fullOnly)assert(full.requested.includes(src),`Full must load ${src}`);for(const src of freeOnly)assert(!full.requested.includes(src),`Full must not load Free-only ${src}`);assert(full.requested.includes('/evidence-intro-dismiss.js?v=2'),'Full must retain the shared dismissible Evidence guide');assert(full.styles.includes(fullStyle),'Full must load the core-owned mobile Evidence grid style');
 ...
 assert.equal(free.composition,'free');for(const src of fullOnly)assert(!free.requested.includes(src),`Free must never load Full-only ${src}`);for(const src of freeOnly)assert(free.requested.includes(src),`Free must retain ${src}`);assert(free.requested.includes('/evidence-intro-dismiss.js?v=2'),'Free must retain the existing shared Evidence guide');assert(!free.styles.includes(fullStyle),'Free must never load Full Evidence grid styling');
