...
const fullOnly=['/girls-vault-contract-fix.js?v=1','/girls-hidden-upload-choice.js?v=1','/girls-media-performance-max.js?v=2','/girls-media-ux-plus.js?v=2','/girls-evidence-parity.js?v=2','/girls-media-quality-fix.js?v=4','/girls-mobile-evidence-grid.js?v=3','/girls-media-readiness.js?v=2','/girls-direct-photo-viewer.js?v=4','/girls-gallery-no-zoom.js?v=1','/girls-media-flow-refinement.js?v=1','/girls-media-social.js?v=1','/video-thumbnail-fix.js?v=1'];
...
try{
 ...
 const full=await evaluate(client,'({requested:window.__requested.slice(),composition:document.querySelector(\'.dashboard\')?.dataset.homeComposition})');
 assert.equal(full.composition,'full');for(const src of fullOnly)assert(full.requested.includes(src),`Full must load ${src}`);for(const src of freeOnly)assert(!full.requested.includes(src),`Full must not load Free-only ${src}`);assert(full.requested.includes('/evidence-intro-dismiss.js?v=2'),'Full must retain the shared dismissible Evidence guide');
 ...
 const free=await evaluate(client,'({requested:window.__requested.slice(),composition:document.querySelector(\'.dashboard\')?.dataset.homeComposition})');
 assert.equal(free.composition,'free');for(const src of fullOnly)assert(!free.requested.includes(src),`Free must never load Full-only ${src}`);for(const src of freeOnly)assert(free.requested.includes(src),`Free must retain ${src}`);assert(free.requested.includes('/evidence-intro-dismiss.js?v=2'),'Free must retain the existing shared Evidence guide');
