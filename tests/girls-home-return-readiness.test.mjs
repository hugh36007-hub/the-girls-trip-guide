...
async function setPage(c,mode){
 ...
 await evalJs(c,`window.__events=[];let firstHomeAsset=true;const append=document.body.appendChild.bind(document.body);document.body.appendChild=function(node){if(node?.tagName==='SCRIPT'&&node.dataset?.gtgDeferred==='1'){node.removeAttribute('src');const out=append(node);const delay=firstHomeAsset?80:0;firstHomeAsset=false;setTimeout(()=>{window.__events.push('home-assets');node.onload?.()},delay);return out}return append(node)};document.addEventListener('click',e=>{if(e.target.closest?.('[data-tab="overview"]')){window.__events.push('activate');document.querySelector('[data-panel=evidence]').classList.remove('active');document.querySelector('[data-panel=overview]').classList.add('active')}});`);
