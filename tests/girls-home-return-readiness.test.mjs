...
async function setPage(c,mode){
 ...
 await evalJs(c,`window.__events=[];const append=document.body.appendChild.bind(document.body);document.body.appendChild=function(node){if(node?.tagName==='SCRIPT'&&node.dataset?.gtgDeferred==='1'){const isHome=node.getAttribute('src')?.includes('home-social-platform-guard.js');node.removeAttribute('src');const out=append(node);setTimeout(()=>{if(isHome)window.__events.push('home-assets');node.onload?.()},isHome?80:0);return out}return append(node)};document.addEventListener('click',e=>{if(e.target.closest?.('[data-tab="overview"]')){window.__events.push('activate');document.querySelector('[data-panel=evidence]').classList.remove('active');document.querySelector('[data-panel=overview]').classList.add('active')}});`);
