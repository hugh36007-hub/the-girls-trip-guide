...
assert(polish.includes('function enhanceBookingPayer()'),'mobile Plan payer controlled picker is missing');
assert(polish.includes("select.dataset.gtgMobilePayerSource='1'"),'native Plan payer select must be replaced only after controlled picker initialises');
assert(polish.includes("select.value=option.value"),'controlled Plan payer picker must write the selected member to the real form field');
assert(polish.includes("sync();close();requestAnimationFrame(()=>trigger.focus({preventScroll:true}))"),'controlled Plan payer picker must visibly update and close after selection');
