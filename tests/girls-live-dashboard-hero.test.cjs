...
assert.match(html,/girls-app-v2\.js\?v=11/,'the private shell must cache-bust the core renderer');
assert.match(html,/girls-hero-vault-ux\.js\?v=4/,'the private shell must cache-bust the retired legacy hero control path');
assert.match(html,/girls-performance-loader\.js\?v=21/,'the private shell must cache-bust the unified runtime loader');
...
assert.match(sw,/gtg-pwa-v40-hidden-gallery-rollback/,'the current Home composition must use the push-capable offline cache generation');
