...
assert.match(critical,/girls-live-dashboard-hero\.js\?v=8/,'the critical loader must cache-bust the corrected runtime');
assert.doesNotMatch(dateFlow,/girls-live-dashboard-hero\.js/,'the legacy date flow must not load a duplicate hero runtime');
assert.match(html,/girls-critical-style-loader\.js\?v=20260908-5/,'the private shell must cache-bust the corrected critical loader');
assert.match(html,/girls-date-flow\.js\?v=3/,'the private shell must cache-bust the duplicate-free date flow');
assert.match(html,/girls-pwa-register\.js\?v=3/,'the private shell must refresh PWA registration code');
assert.match(sw,/gtg-pwa-v9/,'the fixed private shell must replace the previous offline cache generation');
