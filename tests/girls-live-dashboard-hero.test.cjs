...
assert.doesNotMatch(critical,/ensureHomeAssets|GTGCritical|MutationObserver/,'critical loader must be styling-only');
assert.match(performance,/async function openHome\(target=null\)/,'performance loader must own one async Full Home entry gate');
assert.match(performance,/await loadBundle\('home'\)/,'Home entry must wait for the single authoritative Home bundle');
assert.equal((performance.match(/\/girls-live-dashboard-hero\.js/g)||[]).length,1,'Full Home hydrator must have one loader owner');
...
assert.doesNotMatch(performance,/girls-home-hero-background\.js/,'the legacy all-Home dark background runtime must not load');
...
assert.doesNotMatch(critical,/first-paint-cover|finalReady|failsafe/,'Home styling must not block the core dashboard behind an artificial cover');
...
assert.match(html,/girls-app-v2\.js\?v=10/,'the private shell must cache-bust the core renderer');
assert.match(html,/girls-hero-vault-ux\.js\?v=4/,'the private shell must cache-bust the retired legacy hero control path');
assert.match(html,/girls-performance-loader\.js\?v=19/,'the private shell must cache-bust the unified runtime loader');
...
assert.match(sw,/gtg-pwa-v38-hidden-gallery-mobile-touch/,'the current Home composition must use the push-capable offline cache generation');
