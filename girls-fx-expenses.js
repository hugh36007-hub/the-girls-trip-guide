...
async function trip(){const id=tripId(),c=db();if(!id||!c)return null;if(tripCache&&tripCacheId===id)return tripCache;const {data,error}=await c.from('trips').select('id,owner_id,product_key,destination,currency,currency_override').eq('id',id).eq('product_key','girls').maybeSingle();if(error)throw error;tripCache=data||null;tripCacheId=id;return tripCache}
...
async function fxRate(currency,amount){
 ...
 const c=db();if(!c)throw Error('Secure services did not load.');const {data:{session}}=await c.auth.getSession();if(!session?.access_token)throw Error('Your session expired. Sign in again.');
...
async function enhanceExpense(form){
 if(!form||form.dataset.gtgFxReady||form.dataset.gtgFxLoading)return;form.dataset.gtgFxLoading='1';installStyles();
 try{
  const t=await trip();if(!t)return;const id=recordId(form);let existing=null;if(id){const {data}=await db().from('expenses').select('id,amount,original_amount,original_currency,gbp_rate,fx_rate_date,fx_source').eq('id',id).eq('trip_id',t.id).maybeSingle();existing=data||null}
...
function scanModal(){const root=document.getElementById('modalRoot')||document.getElementById('modal-root');if(!root)return;void enhanceExpense(root.querySelector('#expenseForm'));void enhanceTripSettings(root.querySelector('#editTripForm'))}
...
document.addEventListener('submit',event=>{const form=event.target;if(!(form instanceof HTMLFormElement))return;const id=form.getAttribute('id');if(id==='expenseForm'&&form.dataset.gtgFxReady==='1'){event.preventDefault();event.stopImmediatePropagation();void saveExpense(form).catch(err=>{console.error(err);resetBusy(form);toast(err.message||'Could not save expense.')});return}if(id==='editTripForm'&&form.dataset.gtgCurrencyReady==='1'){event.preventDefault();event.stopImmediatePropagation();void saveTripSettings(form).catch(err=>{console.error(err);resetBusy(form);toast(err.message||'Could not save trip settings.')})}},true);
