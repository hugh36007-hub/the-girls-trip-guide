...
function overview(){
 ...
 if(state.paid){
  const overviewPanel=document.querySelector('.dashboard .panel[data-panel="overview"]');if(!overviewPanel)return;
  const head=overviewPanel.querySelector(':scope > .section-head');if(head)head.insertAdjacentElement('afterend',block);else overviewPanel.prepend(block)
 }else{
  const dashboard=document.querySelector('.dashboard');if(!dashboard)return;
  const anchor=document.querySelector('.stat-row');if(anchor)anchor.insertAdjacentElement('afterend',block);else dashboard.prepend(block)
 }
}
