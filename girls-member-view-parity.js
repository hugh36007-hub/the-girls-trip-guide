...
function stripPassportSummaries(){document.querySelectorAll('.gtg-overview-strip span').forEach(span=>{const label=span.querySelector('small')?.textContent||'';if(/passport/i.test(label))span.remove()})}
function normaliseHome(){
 const position=financialPosition(),requests=unpaidRequests();
 const stat=document.querySelector('.stat-row .stat[data-tab="money"]');if(stat){setText(stat.querySelector(':scope>b'),money(position.owes));setText(stat.querySelector(':scope>small'),`${requests.length} payment request${requests.length===1?'':'s'} for you`)}
 const overview=document.querySelector('.gtg-parity-overview');if(overview){
  const cards=[...overview.querySelectorAll('.gtg-overview-card')];if(cards[1]){setText(cards[1].querySelector('.eyebrow'),'Your position');setText(cards[1].querySelector('h2'),position.owes>0?`${money(position.owes)} outstanding`:position.credit>0?`${money(position.credit)} owed to you`:'Settled');setText(cards[1].querySelector('p'),requests.length?`${requests.length} payment request${requests.length===1?'':'s'} currently needs your attention.`:'Nothing currently needs paying.')}
  const summary=overview.querySelector('.gtg-member-summary');if(summary){setText(summary.querySelector('strong'),money(position.owes));setText(summary.querySelector('p'),'Your organiser controls the official trip details. You can read the same trip view and contribute without seeing other members’ private admin or balances.')}
 }
...
function normaliseGroup(){
 ...
 const cards=[...panel.querySelectorAll('.crew-card')];cards.forEach((card,index)=>{
  ...
  setText(card.querySelector('.crew-id small'),`${memberRole(row)}${counts.total?` · ${counts.total} upload${counts.total===1?'':'s'}`:''}`);
  const avatar=card.querySelector('.avatar');if(avatar&&!avatar.dataset.gtgReadonlyProfile){avatar.dataset.gtgReadonlyProfile=row.id;avatar.setAttribute('role','button');avatar.setAttribute('tabindex','0');avatar.setAttribute('aria-label',`View ${row.name||'group member'} profile`)}
...
function renderMoney(){
 ...
 if(panel.querySelector('.gtg-member-money-private'))return;
 const position=financialPosition(),requests=myRequests(),rows=personalRows(),box=document.createElement('div');box.className='gtg-member-money-private';
...
function openUploads(userId){const evidence=document.querySelector('.dock [data-tab="evidence"],.stat-row [data-tab="evidence"]');evidence?.click();setTimeout(()=>{const filter=[...document.querySelectorAll('[data-a="filterEvidence"][data-id]')].find(node=>String(node.dataset.id)===String(userId||''));filter?.click()},60)}
...
async function boot(){if(!window.supabase?.createClient){setTimeout(boot,50);return}await loadContext();if(!ctx||ctx.owner)return;const root=document.getElementById('app')||document.body;new MutationObserver(schedule).observe(root,{childList:true,subtree:true,characterData:true});schedule();window.addEventListener('pageshow',schedule);window.addEventListener('popstate',schedule)}
