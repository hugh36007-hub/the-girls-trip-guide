...
const RESEARCH_PATHS=new Set(['/insights','/group-trip-photo-problem','/hen-do-planner','/girls-weekend-planner','/group-trip-cost-planner','/organise-group-trip-without-whatsapp','/first-100']);
...
.brand.site-logo picture{display:flex!important;align-items:center!important;justify-content:center!important;width:100%!important;height:100%!important}
...
.crumbs,.byline,.article-meta{color:#6c5962!important}
.eyebrow,.section-kicker{color:#c81f70!important}
.footer h3{color:#c81f70!important}
.footer-bottom{color:#6c5962!important}
.mobile-plan-link,.article-link{color:#c81f70!important}
...
@media(max-width:700px){
 ...
 .mobile-plan-link{display:inline-flex!important;align-items:center!important;justify-content:center!important;margin-left:auto!important;min-height:34px!important;padding:0 12px!important;border:1px solid rgba(200,31,112,.72)!important;border-radius:999px!important;color:#c81f70!important;font:900 9px/1 Inter,Arial,sans-serif!important;letter-spacing:.045em!important;text-transform:uppercase!important;white-space:nowrap!important}
 ...
 .mobile-drawer a:last-child{border-bottom:0!important}.mobile-drawer a::after{content:'→';color:#c81f70;font-size:14px}
}`;
...
const forceApprovedHeader=()=>{
  ...
  if(brand){
    ...
    brand.innerHTML='<picture><source media="(max-width:700px)" srcset="/assets/images/hero-trans-mobile.webp" type="image/webp"><img src="/assets/images/hero-trans.png" width="70" height="70" alt="The Girls Trip Guide — Good Plans. Better Stories." loading="eager" decoding="async"></picture>';
    brand.removeAttribute('style');
    const img=brand.querySelector('img');
    if(img)img.removeAttribute('style');
  }
  const navLinks=document.getElementById('navLinks');
  if(navLinks){navLinks.innerHTML='<a href="/situation">So… what’s the plan?</a><a href="/the-gals#how-it-works">How It Works</a><a href="/free-vs-full">Free vs Full</a><a href="/gals">The GALS</a><a href="/first-100">First 100</a><a href="/briefing">The Briefing</a>';navLinks.querySelectorAll('a').forEach(a=>{const href=cleanPath(new URL(a.href,location.origin).pathname);if((path==='/the-gals'&&href==='/the-gals')||path===href){a.classList.add('active');a.setAttribute('aria-current','page')}});}
...
const addFirst100FooterLink=()=>{
  const footer=document.querySelector('.footer');
  if(!footer||footer.querySelector('a[href="/first-100"],a[href="/first-100.html"]')) return;
  const marker=[...footer.querySelectorAll('a')].find(a=>/research & insights/i.test(a.textContent||''))||[...footer.querySelectorAll('a')].find(a=>/sitemap/i.test(a.textContent||''));
  if(!marker||!marker.parentNode)return;
  const link=document.createElement('a');link.href='/first-100';link.textContent='The First 100';marker.parentNode.insertBefore(link,marker);
};
addFirst100FooterLink();
if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',addFirst100FooterLink,{once:true});
