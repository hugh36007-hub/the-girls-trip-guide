...
let timer=0;
function tune(){clearTimeout(timer);timer=setTimeout(()=>{[...document.querySelectorAll('[data-panel="evidence"] .gallery > .media img[data-media-id]')].forEach((image,index)=>{image.decoding='async';image.loading=index<9?'eager':'lazy';try{image.fetchPriority=index<3?'high':'auto'}catch{}})},20)}
